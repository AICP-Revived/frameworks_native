...
status_t Parcel::appendFrom(const Parcel *parcel, size_t offset, size_t len)
{
    ...
    status_t status = validateReadData(mDataPos + len);
    if (status != OK) {
        return status;
    }
...
restart_write:
        ...
        if (status_t status = validateReadData(mDataPos + padded)) {
            if (status != OK) {
...
restart_write:
        if (status_t status = validateReadData(mDataPos + sizeof(val))) {
            if (status != OK) {
                return status;
            }
        }
